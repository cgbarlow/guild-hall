-- Migration: 135_update_email_triggers_granular
-- Description: Update email triggers to check granular preferences

-- Update notify_objective_status_change to check email_quest_updates
...
BEGIN
  ...
  -- Get user info, quest info, and email preferences
  SELECT
    u.email,
    ...
    COALESCE(ps.email_quest_updates, true)
  INTO
    v_user_email,
    v_user_name,
    v_quest_title,
  ...
  -- Skip if email notifications are disabled (master or specific)
  IF NOT v_email_enabled OR NOT v_email_quest_updates THEN
    RETURN NEW;
  END IF;
...
-- Update notify_quest_completed to check email_quest_completion
...
DECLARE
  ...
  v_badge_url TEXT;
  v_email_enabled BOOLEAN;
  v_email_quest_completion BOOLEAN;
BEGIN
  ...
  SELECT
    ...
    q.badge_url,
    COALESCE(ps.email_notifications, true),
    COALESCE(ps.email_quest_completion, true)
  INTO
    v_user_email,
    v_user_name,
    v_quest_title,
  ...
  -- Skip if email notifications are disabled (master or specific)
  IF NOT v_email_enabled OR NOT v_email_quest_completion THEN
    RETURN NEW;
  END IF;
...
-- Update notify_extension_response to check email_quest_updates
...
-- Update notify_banner_email to check email_gm_messages
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(u.display_name, split_part(u.email, '@', 1)),
    COALESCE(ps.email_notifications, true),
    COALESCE(ps.email_gm_messages, true)
  INTO
    v_user_email,
    v_user_name,
    v_email_enabled,
  ...
  -- Skip if email notifications are disabled (master or specific)
  IF NOT v_email_enabled OR NOT v_email_gm_messages THEN
